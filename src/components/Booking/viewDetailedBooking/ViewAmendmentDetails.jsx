...
const ViewAmendmentDetails = ({ amendment }) => {
  ...
  return (
    <div className="mx-3 flex flex-col gap-4 my-4">
      ...
      {modalIsOpen && (
        <div 
          className="fixed inset-0 bg-black bg-opacity-50 flex w-full justify-center items-start z-50 o pt-4 pb-4"
          onClick={handleOverlayClick}
        >
          <div 
            className="bg-gray-200 w-full max-w-4xl m-auto"
                      ...
                      <div className="flex space-x-2 flex-wrap">
                        {trips.map((trip, index) => (
                          <button
                            ...
                            onClick={() => setSingleTripDetails(trip)}
                            className={`${
                              singleTripDetails === trip ? 'bg-yellow-600 text-gray-800' : 'bg-gray-700 text-white'
                            } font-semibold uppercase py-2 px-4 rounded-lg flex flex-col gap-1 mb-2`}
                          >
                            <span>Trip - {index + 1}</span>
                            <span>{trip.src} - {trip.dest}</span>
