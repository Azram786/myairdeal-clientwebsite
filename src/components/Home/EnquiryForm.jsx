...
//     //         // Add the new enquiry to the previous enquiries list
//     //         setPreviousEnquiries((prev) => [...prev, { id: prev.length + 1, ...formData }]);
//     //         setFormData({ email: "", phone: "", dialCode: "", type: "", description: "" });
//     //     }
//     // };
...
//         if (validateForm()) {
//             try {
//                 const response = await axios.post(`${import.meta.env.VITE_SERVER_URL}enquiry/create-registeredUser`, formData, {
//                     headers: {
...
//         try {
//             const responce = await axios.get(`${import.meta.env.VITE_SERVER_URL}enquiry/get-tickets?pageSize=2`, {
//                 headers: {
//                     Authorization: `Bearer ${token}`,
//                 },
//             })
//             console.log({ responce })
//             setPreviousEnquiries(responce.data)
//         } catch (error) {
//             console.log(error.message)
//         }
//     }
...
//                                 border: errors.phone ? "1px solid #E53E3E" : "1px solid #4A5568",
//                                 color: "white",
//                             }}
//                             buttonStyle={{
//                                 backgroundColor: "#282C35",
...
//                             searchPlaceholder="Search countries"  // Custom placeholder for search input
...
//             <h3 className="text-lg font-bold mt-8 text-[#D7B56D]">Previous Enquiries</h3>
//             <div className="mt-4 ">
...
const EnquiryForm = () => {
    ...
    const getEnquiryDetails = async (currentPage = 1, reset = false) => {
        if (isLoading) return;
        setIsLoading(true);
        try {
            const response = await axios.get(`${import.meta.env.VITE_SERVER_URL}enquiry/get-tickets?pageSize=5&page=${currentPage}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            ...
            if (newEnquiries.length > 0) {
                setPreviousEnquiries((prev) => reset ? newEnquiries : [...prev, ...newEnquiries]);
