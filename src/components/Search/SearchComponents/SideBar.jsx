...
const SideBar = ({ flights, filters, setFilters, activeTabIndex, passenger }) => {
  ...
  return (
    <div className="flex flex-row text-xs md:text-base w-[250px] border p-4 m-2 shadow-md rounded-md min-h-screen">
            ...
            {stops.map((stop, index) => {
              ...
              return (
                <label
                  key={stop}
                  htmlFor={`stop-${stop}`}
                  className={`mb-1 border flex hover:bg-[#D7B56D] flex-col text-xs justify-center items-center py-2 ${index === 0 ? 'rounded-l-md' : ''} ${index === stops.length - 1 ? 'rounded-r-md' : ''} ${
                    filters[activeTabIndex].stops.includes(stop) ? 'bg-[#D7B56D]' : ''
                  }`}
                >
                  <input
                    type="checkbox"
        ...
        <div className="mb-6 border-b border-gray-300 pb-4">
          ...
          <div className='grid grid-cols-4 md:grid-cols-2 lg:grid-cols-4 place-items-center gap-2 '>
            {[
              ...
              <span
                key={time}
                className={`border-gray-500 border hover:bg-[#D7B56D] flex text-xs flex-col justify-center items-center rounded-md py-1 w-full cursor-pointer ${
                  filters[activeTabIndex].departureTime.includes(time) ? 'bg-[#D7B56D]' : ''
                }`}
            ...
            {[
              ...
              <span
                ...
                className={`border-gray-500 border hover:bg-[#D7B56D] text-xs flex flex-col justify-center items-center rounded-md py-1 w-full cursor-pointer ${
                  filters[activeTabIndex].arrivalTime.includes(time) ? 'bg-[#D7B56D]' : ''
                }`}
